function plot_choice_reward_single_session_costa(all_stats,representative_ses_idx, species_color,data_idx)
representative_ses_idx = 1;
...
choice = ones(length(stats.c));
reward = ones(length(stats.r));
for i = 1:num_blocks
    tstats = struct;
    tstats.c = stats.c((i-1)*80+1:i*80);
    tstats.r = stats.r((i-1)*80+1:i*80);
    choice((i-1)*80+1:i*80) = movsum(tstats.c==1,avg)./(movsum(tstats.c==-1,avg)+movsum(tstats.c==1,avg));
    reward((i-1)*80+1:i*80) = movsum((tstats.c==1).*tstats.r, avg)./(movsum((tstats.c==-1).*tstats.r, avg)+movsum((tstats.c==1).*tstats.r, avg));
...
for i = 1:length(stats.block_addresses)-1
    if (mod(stats.block_addresses(i),80) == 0)
        xline(stats.block_addresses(i), '-', 'Color', [0 0 0],'LineWidth', 2);
    else
        xline(stats.block_addresses(i), ':', 'Color', [.5 .5 .5],'LineWidth', 2);
    end
end
