function plot_surfaces_supplement(output,point_col)
hold off;
% h = heatmap([1,2]);
% map = h.Colormap;
...
%%Surface plots 
    ...
    hold on;    
    g = scatter3(output.(bd_labels(i,1)),output.(bd_labels(i,2)),output.(bd_labels(i,3)),  'filled', 'MarkerFaceColor', point_col);
    g.SizeData = 20;
    ...
    xax.Position(2) = xax.Position(2) + .05-.04;
    yax.Position(1) = yax.Position(1) + .07-.04;
    
    yax.Position(2) = yax.Position(2) + .05+.02;
    xax.Position(1) = xax.Position(1) -.1+.02;
end
...
for i=5:6
 ...
 ERDS = -(WS.*win.*log2(WS)+(1-WS).*win.*log2(1-WS)...
    ...
    yax.Position(2) = yax.Position(2) -.15+.02;
    xax.Position(1) = xax.Position(1) -.1+.02;
end
figure('Position', [215.6666666666667,958.3333333333333,1564,678.6666666666667]); hold on;
for i=1:4
%%Surface plots 
    if i<3
        subplot(2,3,i+1);
    ...
    hold on;
    g = scatter3(output.(bd_labels(i,1)),output.(bd_labels(i,2)),ones(size(output.(bd_labels(i,2)),1), size(output.(bd_labels(i,2)),2)),'filled','MarkerFaceColor', point_col);
    g.SizeData = 20;
    set(gca, 'XTick', [0:.5:1], 'YTick', [0:.5:1], 'ZTick', [0:.5:1]);
    set(gca,'FontName','Helvetica','FontSize',12);
